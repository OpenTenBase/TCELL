...
int SM3(unsigned char *msg, unsigned int msglen, unsigned int *digest)
{
  ...
  if (!digest)
  {
    printk(KERN_INFO "Failed to kmalloc digest\n");
  }
  SM3_init(&context);
  SM3_update(&context, msg, msglen);
 ...
 */
void SM3_hmac_finish(sm3_context *ctx, UINT32 *output)
{
  int hlen;
  // BYTE tmpbuf[32];
  UINT32 tmpbuf[32];
  ...
  SM3_update(ctx, (unsigned char *)tmpbuf, DIGEST_SIZE);
